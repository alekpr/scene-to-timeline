...
describe("validator", () => {
  ...
  it("accepts optional transcript parameter", async () => {
    const result = await validateAndPrepareInput({
      scene: "A woman walks in an alley",
      duration: "10",
      transcript: "Here is the voiceover... speaking about the scene",
    });

    expect(result.voiceoverTranscript).toBe("Here is the voiceover... speaking about the scene");
  });
